<template>
	<md-preview ref="editorRef" editorId="preview-only" :modelValue="item" v-for="(item, index) in md_view_list" :key="index" class="maxkb-md" />
</template>
...
const md_view_list = computed(() => {
	...
	const result = Array.from({ length: md_img_list.length + split_img_value.length }, (v, i) => i).map((index) => {
		if (index % 2 == 0) {
